// §prose jest guard — same scanning approach as parse/__tests__/adapterGuard.test.ts
// and maps/__tests__/adapterGuard.test.ts: no test file may import the prose
// LLM adapter or the Anthropic SDK directly, so a jest run can never make a
// live (billed) API call.
...
describe("prose adapter import guard", () => {
  ...
  it("no test file imports the explainTradeoffs llm adapter", () => {
    // covers static import, require(), and dynamic import()
    const importPattern = /(?:from\s+|require\(|import\()\s*["'][^"']*explainTradeoffsLlmAdapter["']/;
