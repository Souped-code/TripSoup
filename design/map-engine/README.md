...
| File | What it is |
|---|---|
| `../../src/lib/map/map-render-core.js` | **The engine core** (browser ES module) — MOVED to src/ at M1: it is the PRODUCT's render engine now (the reveal at `/trip/[id]` uses it via `src/ui/reveal/RevealMap.tsx`), and both bench tools serve it from there at `/map-render-core.js`. API: `provideLibs` (app injects npm libs) / `preloadLibs` (bench CDN fallback), `fetchAndDecode(config) -> decoded` (OpenFreeMap MVT fetch+decode, cached per view), `buildScene` → `paintOverlay` → `renderToDisplay` (base painted once; route/pins/washi redraw per visit order), `paintFull(canvas, config, decoded, textures) -> stats` (bench one-shot), `upgradeConfig` (old Copy-CONFIG migration). |
| `../../src/lib/map/map-style-defaults.mjs` | **The single style source** (M0.5 lock target): every ART value (colors/widths/pins/washi/labels), shared by the app, this harness, and the studio. View fields (Z/SCALE/BBOX/VIEW_BBOX/ROUTE_POINTS/WASHI_INDEX) stay per-caller. |
| `render-engine.mjs` | Node+Playwright **screenshot harness**: renders one JB view + a side-by-side vs the board. `node render-engine.mjs` → `render-engine-v0.png` + `render-engine-v0-vs-board.png`. |
